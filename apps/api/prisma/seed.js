/**
 * Predial360 — Seed de demo (vanilla JS, roda no runner Alpine sem ts-node)
 * Credenciais: admin@predial360.com.br / Demo@2025!
 *              joao.silva@email.com    / Demo@2025!
 *              carlos.tech@...        / Demo@2025!
 */
'use strict';

const { PrismaClient } = require('@prisma/client');
const bcrypt = require('bcryptjs');

const prisma = new PrismaClient();
const hash = (pw) => bcrypt.hashSync(pw, 10);
const PASS = 'Demo@2025!';

async function main() {
  // Idempotente: só roda se ainda não existir admin
  const existing = await prisma.user.findFirst({ where: { email: 'admin@predial360.com.br' } });
  if (existing) {
    console.log('  ℹ️  Seed já executado anteriormente. Pulando.');
    return;
  }

  console.log('🌱 Seed de demonstração iniciando...');

  const admin = await prisma.user.create({
    data: {
      email: 'admin@predial360.com.br',
      passwordHash: hash(PASS),
      name: 'Administrador Predial360',
      phone: '(11) 99999-0000',
      role: 'ADMIN',
      status: 'ACTIVE',
      consentGivenAt: new Date(),
    },
  });

  const owner = await prisma.user.create({
    data: {
      email: 'joao.silva@email.com',
      passwordHash: hash(PASS),
      name: 'João Silva',
      phone: '(11) 91234-5678',
      cpf: '123.456.789-00',
      role: 'OWNER',
      status: 'ACTIVE',
      consentGivenAt: new Date(),
    },
  });

  const technician = await prisma.user.create({
    data: {
      email: 'carlos.tech@predial360.com.br',
      passwordHash: hash(PASS),
      name: 'Carlos Eduardo Técnico',
      phone: '(11) 98765-4321',
      crea: 'CREA-SP 123456-D',
      role: 'TECHNICIAN',
      status: 'ACTIVE',
      rating: 4.8,
      consentGivenAt: new Date(),
    },
  });

  const property = await prisma.property.create({
    data: {
      ownerId: owner.id,
      name: 'Edifício Paulista Premium',
      type: 'COMMERCIAL',
      address: 'Av. Paulista, 1578',
      city: 'São Paulo',
      state: 'SP',
      zipCode: '01310-200',
      latitude: -23.5614,
      longitude: -46.6557,
      totalArea: 4500,
      builtArea: 3800,
      floors: 15,
      units: 120,
      yearBuilt: 2005,
      registrationNumber: 'SP-12345-67',
    },
  });

  const asset = await prisma.asset.create({
    data: {
      propertyId: property.id,
      name: 'Sistema de Elevadores — Bloco A',
      category: 'ELEVATORS',
      brand: 'Thyssenkrupp',
      model: 'Evolution 200',
      serialNumber: 'TK-EV-2005-001',
      installationDate: new Date('2005-06-15'),
      warrantyExpiration: new Date('2015-06-15'),
      maintenanceFrequency: 'MONTHLY',
      nextMaintenanceDate: new Date(Date.now() + 7 * 24 * 3600 * 1000),
      notes: 'Revisão semestral obrigatória — ABNT NBR 5674',
      qrCode: 'tk-ev-2005-001-qr',
    },
  });

  const order = await prisma.serviceOrder.create({
    data: {
      code: `OS-${new Date().getFullYear()}-00001`,
      propertyId: property.id,
      ownerId: owner.id,
      technicianId: technician.id,
      assetId: asset.id,
      type: 'PREVENTIVE',
      status: 'ASSIGNED',
      priority: 'HIGH',
      title: 'Manutenção Preventiva — Elevadores (NBR 5674)',
      description: 'Inspeção completa dos sistemas de elevação conforme ABNT NBR 5674. Verificar cabos, freios, painéis de controle e sistemas de segurança.',
      scheduledDate: new Date(Date.now() + 2 * 24 * 3600 * 1000),
      estimatedDurationMinutes: 240,
      estimatedCost: 2800,
    },
  });

  await prisma.contract.create({
    data: {
      ownerId: owner.id,
      propertyId: property.id,
      plan: 'PROFESSIONAL',
      status: 'ACTIVE',
      startDate: new Date(),
      endDate: new Date(Date.now() + 365 * 24 * 3600 * 1000),
      monthlyAmount: 299,
      autoRenew: true,
    },
  });

  console.log('  ✓ Admin:', admin.email);
  console.log('  ✓ Proprietário:', owner.email);
  console.log('  ✓ Técnico:', technician.email);
  console.log('  ✓ Imóvel:', property.name);
  console.log('  ✓ Ativo:', asset.name);
  console.log('  ✓ OS:', order.code);
  console.log('\n✅ Seed de demonstração concluído!');
  console.log('   Admin:     admin@predial360.com.br / Demo@2025!');
  console.log('   Owner:     joao.silva@email.com / Demo@2025!');
  console.log('   Técnico:   carlos.tech@predial360.com.br / Demo@2025!');
}

main()
  .catch((e) => { console.error('Seed error:', e); process.exit(1); })
  .finally(() => prisma.$disconnect());
