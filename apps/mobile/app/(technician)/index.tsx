...
export default function TechnicianHome() {
  ...
  useEffect(() => { navigation.setOptions({ title: 'Minhas Ordens' }); }, [navigation]);
  ...
  if (isError) return <View style={styles.center}><Text style={styles.error}>Erro ao carregar ordens.</Text></View>;
  ...
  return (
    <View style={styles.container}>
      ...
      {orders.length === 0 ? (
        <View style={styles.center}><Text style={styles.empty}>Nenhuma ordem atribuída.</Text></View>
      ) : (
        <FlatList
          data={orders}
          keyExtractor={(o) => o.id}
          renderItem={({ item }) => {
            ...
            return (
              <View style={styles.card}>
                ...
                {item.property && (
                  <Text style={styles.property}>📍 {item.property.name} — {item.property.city}/{item.property.state}</Text>
                )}
